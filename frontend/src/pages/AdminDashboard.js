...
const SectionTitle = ({ children }) => (
  <h2
    style={{ fontSize: 15, fontWeight: 700, color: TEXT, margin: '0 0 18px', letterSpacing: 0.1 }}
  >
    {children}
  </h2>
);
...
const AdminDashboard = () => {
  ...
  const openEdit = (p) => {
    ...
    setFormData({
      ...
      colour: p.colour || '',
      available_sizes: p.available_sizes || emptyForm.available_sizes,
    });
    setShowModal(true);
  };
  const handleSave = async () => {
    try {
      ...
      showToast('danger', 'Save failed — check all fields.');
    }
  };
        ...
        {TABS.map((t) => (
          <button
            ...
            style={{
              ...
              border: 'none',
              borderBottom:
                activeTab === t.key ? `2px solid ${ACCENT_PURPLE}` : '2px solid transparent',
              color: activeTab === t.key ? ACCENT_PURPLE : MUTED,
          ...
          <>
            ...
            >
              {[
                {
                  label: 'Total Revenue',
                  val: formatCurrency(stats.total_revenue),
                  sub: 'all time',
                },
                ...
                <Card key={i} padding="20px 22px">
                  ...
                  <div style={{ fontSize: 28, fontWeight: 800, color: TEXT, lineHeight: 1.1 }}>
                    {c.val}
                  </div>
            ...
            {analytics?.monthly_sales?.length > 0 && (
              <Card style={{ marginBottom: 20 }}>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart
                    data={analytics.monthly_sales.map((d) => ({
                      ...d,
          ...
          <>
            ...
            <Card style={{ marginBottom: 20 }}>
              ...
              <ResponsiveContainer width="100%" height={260}>
                <BarChart
                  data={analytics.monthly_sales.map((d) => ({ ...d, month: monthShort(d.month) }))}
                  barSize={32}
                >
                  <CartesianGrid strokeDasharray="3 3" stroke={BORDER} vertical={false} />
                  <XAxis
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                  ...
                  <tbody>
                    {filteredLog.length === 0 ? (
                      <tr>
                        <td colSpan={13} style={{ textAlign: 'center', padding: 36, color: MUTED }}>
                      ...
                      filteredLog.map((r, i) => (
                        ...
                        >
                          ...
                          <td style={{ padding: '10px 14px' }}>
                            {pill(r.brand, '#f3f0ff', ACCENT_PURPLE)}
                          </td>
                          <td style={{ padding: '10px 14px', color: MUTED, fontSize: 12 }}>
                            {r.colour}
                          </td>
                          ...
                          <td style={{ padding: '10px 14px' }}>
                            <StatusPill status={r.order_status} />
                          </td>
                        </tr>
                      ))
                    )}
            ...
            <Card padding="0">
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                  <thead>
                    <tr style={{ background: PAGE_BG, borderBottom: `1px solid ${BORDER}` }}>
                      {['Order #', 'Date', 'Customer', 'Items', 'Total', 'Payment', 'Status'].map(
                        (h) => (
                          <th
                            key={h}
                            style={{
                  ...
                  <tbody>
                    {(stats?.recent_orders || []).map((o, i) => (
                      ...
                      >
                        ...
                        <td style={{ padding: '11px 16px', color: MUTED, maxWidth: 260 }}>
                          {(o.items || []).map((it) => `${it.name} (${it.size})`).join(', ')}
                        </td>
          ...
          >
            ...
            >
              <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700 }}>
                {editingProduct ? 'Edit sneaker' : 'Add sneaker'}
              </h3>
              <button
            ...
            <div style={{ padding: 22, display: 'flex', flexDirection: 'column', gap: 14 }}>
              {[
                ...
                },
                {
                  label: 'Image path',
                  key: 'image',
                  placeholder: '/media/sneakers/images/file.jpg',
                },
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData((fd) => ({ ...fd, gender: e.target.value }))}
                  ...
                  {Object.keys(formData.available_sizes).map((s) => (
                    <button
                      ...
                      onClick={() =>
                        setFormData((fd) => ({
                          ...fd,
                          available_sizes: { ...fd.available_sizes, [s]: !fd.available_sizes[s] },
                        }))
                      }
                      style={{
                        padding: '6px 13px',
                        borderRadius: 6,
                        border: `1px solid ${formData.available_sizes[s] ? ACCENT_PURPLE : BORDER}`,
                        background: formData.available_sizes[s] ? '#f3f0ff' : CARD_BG,
                        color: formData.available_sizes[s] ? ACCENT_PURPLE : MUTED,
                        fontWeight: 600,
                        cursor: 'pointer',
                        fontSize: 13,
                      }}
            ...
            >
              ...
              >
                {editingProduct ? 'Save changes' : 'Add sneaker'}
              </button>
            </div>
          </div>
        </div>
